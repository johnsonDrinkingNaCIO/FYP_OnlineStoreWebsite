...
function CardBody(
  ...
  const renderRightSideButtons = () => {
    // Check if rightSideBtns is provided and has elements
    if (rightSideBtns && rightSideBtns.length > 0) {
      return rightSideBtns.map((buttonProps, index) => (
        <button key={index} {...buttonProps} className={`flex-item ${buttonProps.className}`}>
          {buttonProps.label}
        </button>
      ));
    } else {
      // Default buttons if no props provided
      return (
        <>
  ...
  return (
    <div className={cardBodyStyle} ref={cardBody} >
